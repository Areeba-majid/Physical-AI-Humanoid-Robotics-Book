...
- Grounded Responses Only: [Check that responses are based on book content]
- Source-Scoped Answering: [Check that responses respect text boundaries when specified]
- Architectural Discipline: [Check that layers are properly separated]
- Accuracy Over Fluency: [Check that precision is valued over verbosity]
- Security & Isolation: [Check that data doesn't leak across contexts]
- Educational Quality: [Check that responses are suitable for learners]
...
- How does the system handle insufficient context in the selected text?
...
<!--
  ...
  IMPORTANT: All requirements must align with the constitution principles.
-->
...
- **FR-007**: Users MUST be able to [key interaction, e.g., "reset their password"]
- **FR-008**: System MUST [data requirement, e.g., "persist user preferences"]
- **FR-009**: System MUST [behavior, e.g., "log all security events"]
...
- **SC-005**: [Constitution compliance metric, e.g., "100% of responses are grounded in book content without fabrication"]
